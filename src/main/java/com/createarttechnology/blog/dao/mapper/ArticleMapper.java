...
public interface ArticleMapper {
    ...
    List<ArticleEntity> getArticleList() throws Exception;

    @Insert("INSERT INTO article(title, simple_content, rich_content, tag, pics, create_time, update_time) VALUES " +
            "(#{article.title}, #{article.simpleContent}, #{article.richContent}, #{article.tag}, #{article.pics}, NOW(), NOW())")
    ...
    @Update("UPDATE article SET title = #{article.title}, simple_content = #{article.simpleContent}, rich_content = #{article.richContent}, tag = #{article.tag}, pics = #{article.pics}, update_time = NOW() WHERE id = #{id}")
